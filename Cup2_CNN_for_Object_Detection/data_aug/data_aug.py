...
class RandomHorizontalFlip(object):
    ...
    def __call__(self, img, bboxes):
            ...
            if random.random() < self.p:
                img = img[:, ::-1, :]
                bboxes[:, [0, 2]] += 2*(img_center[[0, 2]] - bboxes[:, [0, 2]]).astype(int)
...
class RandomScale(object):
    ...
    def __call__(self, img, bboxes):
        ...
        bboxes[:, :4] = (bboxes[:, :4] * np.array([resize_scale_x, resize_scale_y, resize_scale_x, resize_scale_y])).astype(bboxes.dtype)
...
class RandomShear(object):
    ...
    def __call__(self, img, bboxes):
        ...
        bboxes[:, :4] /= np.array([scale_factor_x, 1, scale_factor_x, 1], dtype=bboxes.dtype)
